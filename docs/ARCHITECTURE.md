...
Web 复用同一 Camera Lab，适合快速试用和轻量创作。端侧大模型、超大图和完整工程文件仍优先在原生端实现。
...
Camera Lab 历史最多保留 32 个不可变状态快照。快照共享同一份导入图片字节；连续滑块拖动通过事务合并为一个历史步骤，避免高频操作造成内存膨胀。
...
- 小于 1100dp：单列移动/平板布局，底部工具栏和抽屉面板。
- 1100dp 及以上：三栏桌面工作区。
...
- 预览与最终导出分离。
- 当前 PNG 导出由主预览的 `RepaintBoundary` 生成；Web 使用 Blob 下载，桌面使用系统保存面板，移动端使用系统分享面板。
- 移动端使用降采样预览和分块导出。
